...
const navigate = useNavigate();
  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await fetch('https://amaghara-server.onrender.com/auth/user/logout', {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
                setUser(null); // This is the key fix - set user to null
              ...
              {loading ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="hidden md:block">
                    <div className="h-4 w-20 bg-indigo-100 rounded animate-pulse mb-1"></div>
                    <div className="h-3 w-16 bg-indigo-100 rounded animate-pulse"></div>
                  </div>
                </div>
              ) : user ? (
                // Profile Dropdown
              ...
              <div className="mx-auto max-w-7xl px-6 py-4 grid gap-2">
                ...
                {loading ? (
                  <div className="grid gap-2 mt-4 pt-4 border-t border-indigo-100">
                    <div className="h-10 bg-indigo-100 rounded-lg animate-pulse"></div>
                    <div className="h-10 bg-indigo-100 rounded-lg animate-pulse"></div>
                  </div>
                ) : user ? (
                  <div className="grid gap-2 mt-4 pt-4 border-t border-indigo-100">
                    ...
                    <button
                      ...
                      disabled={logoutLoading}
                      className="flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-red-600 hover:bg-red-50 text-left disabled:opacity-50"
                    >
                      {logoutLoading ? (
                        <>
                          <FaSpinner className="w-4 h-4 animate-spin" />
